-- ============================================================
-- AgentScan — corrected schema for REAL ERC-8004 + ERC-8183 events
-- ============================================================
-- The original agentscan_postgresql_schema.sql (agents/quotes/escrows/
...
-- the indexer actually writes to, matching agentscan_indexer.go after the
-- ABI rewrite documented in AUDIT.md.
--
-- NOT YET DONE (see AUDIT.md "Still open" section): the GraphQL schema
-- (schema.graphqls), the API resolvers (agentscan_api.go), the React
...
-- ── Risk / fraud detection (AgentScan's differentiator — see AUDIT.md) ──
-- Populated by agentscan_riskscan.go's periodic scans, not by indexing
-- on-chain events directly. This is AgentScan's own derived analysis,
