# AgentScan — Handoff: Layer 2 (GraphQL / API / Frontend / Seed)
...
You are continuing work on AgentScan, an X Layer agent-economy explorer.
The indexer (`agentscan_indexer.go`) and its Postgres schema
...
- **The new Postgres schema**: `schema_erc8004_8183.sql` — tables `agents`,
  ...
  should read from going forward — `agentscan_postgresql_schema.sql` is
...
- Keep subscriptions (WebSocket) working — re-point them at the new
  ...
  `agentscan_api.go`'s existing subscription/pubsub wiring before assuming
...
## Task 2 — API resolvers (`agentscan_api.go`)
