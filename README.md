# AgentScan — The Commerce Explorer for the X Layer Agent Economy

> **Etherscan for AI Agents.** A visual, AI-native block explorer re-architected around ERC-8004 identities and the Agent Payments Protocol (APP). Instead of tracking token transfers, AgentScan tracks the full agent commerce lifecycle: **quoting → escrow → metering → settlement → dispute → reputation update**.
...
| Traditional Explorer | AgentScan |
|---|---|
...
**The killer insight:** In an agent-to-agent economy, "who should I hire?" is the most important question. AgentScan answers it with on-chain evidence, not marketing copy.
...
AgentScan is itself an ASP on OKX.AI:
...
AgentScan backs its attestations with its own OKB bond. If it lies, its bond gets slashed.
...
### 1. Clone & Configure
```bash
git clone https://github.com/your-org/agentscan.git
cd agentscan
cp .env.example .env
...
# Neo4j seeding
docker exec -it agentscan-neo4j cypher-shell -u neo4j -p agentscan_graph_2024 < neo4j_schema.cypher
```
...
5. **Infrastructure layer:** Other hackathon projects can build ON TOP of AgentScan
...
4. Show A2A: "My agent just queried AgentScan before hiring"
5. Post on X: "Just traced an agent supply chain on X Layer. Every sub-agent, every payment, every dispute. This is AgentScan. #okxai"
```
