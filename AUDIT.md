...
**There is no confirmed real contract for AgentScan to index yet.** This is
...
2. Pivot AgentScan's real-time indexing to track actual settlement
...
The original `agentscan_postgresql_schema.sql` was built around the
...
- `agentscan_api.go` — all resolvers and REST handlers rewritten against
  ...
  `agentscan_indexer.go`'s handlers.
...
## New — risk/fraud detection layer (AgentScan's actual differentiator)
...
the community), agentscan.info (Alias.AI), and RNWY (4.5M+ commerce jobs
...
searched. That's what `agentscan_riskscan.go` (new file) does — three
periodic checks:
...
1. **Circular hiring detection** — agents hiring each other in a closed
   ...
   `agentscan_neo4j_schema.cypher`, itself rewritten in this pass — it was
...
**Fixed:** files moved into `indexer/` (`agentscan_indexer.go` +
`agentscan_riskscan.go` — one binary, since `agentscan_riskscan.go` needs
the indexer's live DB/Neo4j/Redis connections), `api/`
(`agentscan_api.go` + `schema.graphqls`), and `frontend/src/`
...
caught one real compile bug this way: `agentscan_api.go` used
...
`schema_erc8004_8183.sql`/`agentscan_postgresql_schema.sql` and
`agentscan_neo4j_schema.cypher`). This was broken from the very first
...
`/api/v1/risk-alerts` every 30s — putting AgentScan's actual differentiator
